...
SORT_BY_COLS = True  # True → sort by COLS first, then SEARCH_COLUMN
COLS = ["Method"]

# MINIMIZE is auto-determined: RPE/ATE/Failures → minimize, all others → maximize.
...
IDENTITY_COLS = [
    ...
    "Gaussian blur", "Image transformation type", "Distance error threshold",
]

SELECT = {
    # "Method"                    : ["SIFT", "ORB", "BRISK", "AKAZE", "SHIFT"],
    ...
    "Distance error threshold"  : {"vals": list(range(1, 11)), "fn": "auc"},
}
...
if MINIMIZE is None:
    _minimize = _SEARCH_LABEL in _MINIMIZE_COLS
else:
    _minimize = bool(MINIMIZE)
...
if df.empty:
    ...
    ascending = ([True] * len(COLS) + [_minimize]) if SORT_BY_COLS else [_minimize]
    best_rows = (
        df.loc[(agg.idxmin() if _minimize else agg.idxmax()).values]
    ...
    # AUC suffix for mma metric columns (only when Distance error threshold is AUC'd)
    auc_suffix = ""
    ...
    config_lines.append(f"search: {_col_hdr(_SEARCH_LABEL)}  ({'min' if _minimize else 'max'})")
    ...
    n_sticky = len(COLS) + 1
    n_cols = len(COLS)
    minimize_js = "true" if _minimize else "false"
