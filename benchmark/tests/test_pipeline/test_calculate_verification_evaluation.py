from benchmark.pipeline import calculate_verification_evaluation
...
def test_invalid_arguments_calculate_verification_evaluation(sample_feature_extractor, sample_image_feature_set, sample_matching_approach, bad_argument):
    ...
    with pytest.raises((BeartypeCallHintParamViolation, TypeError)):
        calculate_verification_evaluation(feature_extractor, image_feature_set, correct_to_random_ratio, matching_approach)


### Test that valid arguments pass ###

def test_valid_arguments_calculate_verification_evaluation(sample_feature_extractor, sample_image_feature_set, sample_matching_approach):
    calculate_verification_evaluation(sample_feature_extractor, sample_image_feature_set, CORRECT_TO_RANDOM_RATIO, sample_matching_approach)


### Test that general cases behave expectedly ###

@pytest.mark.parametrize("run", range(10))
def test_expected_calculate_verification_evaluation(sample_feature_extractor, sample_image_feature_set, sample_matching_approach, run):

    checked_match_with_both_image_indexes_0 = False
    while not checked_match_with_both_image_indexes_0:
        match_sets = calculate_verification_evaluation(sample_feature_extractor, sample_image_feature_set, CORRECT_TO_RANDOM_RATIO, sample_matching_approach)

        for _, match_set in enumerate(match_sets):
            assert len(match_set) == NUM_FEATURES * (NUM_RELATED_IMAGES + NUM_RELATED_IMAGES * CORRECT_TO_RANDOM_RATIO)
            for match in match_set:
                ...
                if match.related_feature.image_index == 0:
                    assert match.related_feature.sequence_index != match.reference_feature.sequence_index, "If a tested feature is tested against the reference feature, that other feature cannot be from the same reference image"
                    checked_match_with_both_image_indexes_0 = True