    ...
    for downsample_level in tqdm(DOWNSAMPLE_LEVELS, leave=False, desc=f" {feature_extractor_key} Calculating for all levels of downsampling"):
        for max_features in tqdm(MAX_FEATURES_LIST, leave=False, desc="Calculating for all max feature limits"):
            for initial_sigma in tqdm(INITIAL_SIGMAS, leave=False, desc="Calculating for all initial sigmas"):
                with optional_try(SKIP_AT_ERROR, f"{feature_extractor_key}_{RUN_TAG}_{downsample_level}"):
                    ...
                    if "matching" not in SKIP:
                        matching_match_sets: list[MatchSet] = calculate_matching_evaluation(feature_extractor, image_feature_set, matching_approach, USE_MNN, dataset_image_sequences, dataset_homography_sequence, VISUALIZE, SEQUENCE_TO_VISUALIZE, downsample_level, DOWNSAMPLE_FACTOR, INTRINSIC_SIGMA, initial_sigma, APPLY_PROGRESSIVE_BLUR, DOWNSAMPLE_INTERPOLATION_TYPE)
                    else:
                        matching_match_sets: list[MatchSet] = [MatchSet()]
                    
                    if "verification" not in SKIP:
                        verification_match_sets: list [MatchSet] = calculate_verification_evaluation(feature_extractor, image_feature_set, VERIFICATION_CORRECT_TO_RANDOM_RATIO, matching_approach, USE_MNN)
                    else:
                        verification_match_sets: list [MatchSet] = [MatchSet()]
                    
                    if "retrieval" not in SKIP:
                        retrieval_match_sets : list[MatchSet] = calculate_retrieval_evaluation(feature_extractor, image_feature_set, RETRIEVAL_CORRECT_TO_RANDOM_RATIO, MAX_NUM_RETRIEVAL_FEATURES, matching_approach, USE_MNN)
                    else:
                    ...
                    results = {
                        "method":  f"{feature_extractor_key}",
                        "tag": RUN_TAG,
                        ...
                        "avg_num_keypoints" : np.mean([len(image) for sequence in image_feature_set for image in sequence]),
                        "avg_num_missing_keypoints" : np.mean([max_features - len(image) for sequence in image_feature_set for image in sequence]),
                        "avg_num_matches": np.mean([len(s) for s in matching_match_sets])/5,
                    ...
                    results_illumination = results.copy()
                    results_illumination["transformation"] = "illumination"
                    ...
                    if "verification" not in SKIP:
                        ...
                        for match_ranking_property in match_properties:
                            ...
                            results_illumination[f"Verification {match_ranking_property.name} AP"] = AP_illumination
                            results_viewpoint[f"Verification {match_ranking_property.name} AP"] = AP_viewpoint
                    ...
                    if "retrieval" not in SKIP:
                        for match_ranking_property in match_properties:
                            ...
                            results_illumination[f"Retrieval {match_ranking_property.name} mAP"] =  mAP_illumination
